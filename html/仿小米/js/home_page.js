...
//文字导航
//获取元素
var font_nav_l_ul = document.getElementsByClassName("font_nav_l_ul")[0];
var font_nav_l_li = font_nav_l_ul.children;
var font_nav_r = document.getElementsByClassName("font_nav_r")[0];
var font_nav_div = font_nav_r.children;
var font_nav = document.getElementsByClassName("font_nav")[0];
//for循环遍历元素
for (var i = 0; i < font_nav_l_li.length; i++) {
    //给index赋值
    font_nav_l_li[i].index = i;
    //绑定鼠标移入事件
    font_nav_l_li[i].onmouseover = function () {
        //for循环遍历元素
        for (var k = 0; k < font_nav_l_li.length; k++) {
            //清除元素类名
            font_nav_l_li[k].className = "";
            font_nav_div[k].className = "";
        }
        //定义当前元素类名
        font_nav_r.style.display = "block";
        this.className = "font_active";
        font_nav_div[this.index].className = "font_nav_r_avtive";
    }
}
//绑定鼠标移出事件
font_nav.onmouseout = function () {
    var event = event || window.event;//兼容写法
    //获取鼠标x y
    var x = event.clientX;
    var y = event.clientY;
    //判断
    if (x < 580 || x > 1200 || y > 500 || y < 100) {
        //for循环遍历元素
        for (var k = 0; k < font_nav_l_li.length; k++) {
            //清除类名
            font_nav_l_li[k].className = "";
...
//轮播图
//获取元素
var font_lbt = document.getElementsByClassName("font_lbt")[0];
var font_lbt_photo = document.getElementsByClassName("font_lbt_photo")[0];
var li1 = font_lbt_photo.getElementsByTagName("li");
var font_lbt_yuan = document.getElementsByClassName("font_lbt_yuan")[0];
...
//左右点击切换图片 获取左右按钮元素
var right = document.getElementsByClassName("font_lbt_jt_r")[0];
var left = document.getElementsByClassName("font_lbt_jt_l")[0];
//右 给右按钮绑定点击事件
...
//家电
//属性获取
var content_jiadian_header_r = document.getElementsByClassName("content_jiadian_header_r")[0];
var content_jiadian_li = content_jiadian_header_r.children;
var content_phone_r = document.getElementsByClassName("content_jiadian_r")[0];
var content_jiandian_ul = content_phone_r.children;
//for 循环遍历元素
for (var i = 0; i < content_jiadian_li.length; i++) {
    //index 赋值
    content_jiadian_li[i].index = i;
    //绑定鼠标移入事件
    content_jiadian_li[i].onmouseover = function () {
        //for循环遍历元素
        for (var k = 0; k < content_jiadian_li.length; k++) {
            //清除类名
            content_jiadian_li[k].className = "";
            content_jiandian_ul[k].classList.remove("content_jiadian_r_active");
        }
        //定义类名
        this.className = "content_jiadian_active";
        content_jiandian_ul[this.index].classList.add("content_jiadian_r_active");
    }
}
